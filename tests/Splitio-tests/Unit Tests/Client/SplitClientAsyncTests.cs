...
namespace Splitio_Tests.Unit_Tests.Client
{
    ...
    public class SplitClientAsyncTests
    {
        ...
        private readonly Mock<IFallbackTreatmentCalculator> _fallbackTreatmentCalculation;
        ...
        public SplitClientAsyncTests()
        {
            ...
            _splitClient = new SplitClientForTesting(_splitCache.Object, _eventsLog.Object, _impressionsLog.Object, _blockUntilReadyService.Object, _evaluator.Object, _impressionsManager.Object, _syncManager.Object, _telemetryEvaluationProducer.Object, _fallbackTreatmentCalculation.Object);
        }
        ...
        public async Task GetTreatment_WhenNameDoesntExist_ReturnsControl()
        {
            ...
            _evaluator
                ...
                .ReturnsAsync(new List<TreatmentResult> { new TreatmentResult("not_exist", Labels.SplitNotFound, "control", false, null) });
        ...
        public async Task GetTreatments_ShouldReturnTreatments()
        {
            ...
            _evaluator
                ...
                .ReturnsAsync(new List<TreatmentResult>
                {
                    new TreatmentResult(treatmenOff, "label", "off", false, null, configExpectedOff),
                    new TreatmentResult(treatmenOn, "label", "on", false, null, configExpectedOn)
                });
        ...
        public async Task GetTreatments_WhenNameDoesntExist_ReturnsControl()
        {
            ...
            _evaluator
                .SetupSequence(mock => mock.EvaluateFeaturesAsync(It.IsAny<Splitio.Enums.API>(), It.IsAny<Key>(), It.IsAny<List<string>>(), It.IsAny<Dictionary<string, object>>(), true))
                .ReturnsAsync(new List<TreatmentResult> { new TreatmentResult("control_treatment", Labels.SplitNotFound, "control", false, null) });
        ...
        public async Task GetTreatmentWithConfig_WhenConfigIsNull_ShouldReturnOn()
        {
            ...
            _evaluator
                .Setup(mock => mock.EvaluateFeaturesAsync(It.IsAny<Splitio.Enums.API>(), It.IsAny<Key>(), It.IsAny<List<string>>(), It.IsAny<Dictionary<string, object>>(), true))
                .ReturnsAsync(new List<TreatmentResult> { new TreatmentResult(feature, "label", treatmentExpected, false, null) });
        ...
        public async Task GetTreatmentWithConfig_WhenConditionTypeIsWhitelist_ShouldReturnOntWithConfig()
        {
            ...
            _evaluator
                .Setup(mock => mock.EvaluateFeaturesAsync(It.IsAny<Splitio.Enums.API>(), It.IsAny<Key>(), It.IsAny<List<string>>(), It.IsAny<Dictionary<string, object>>(), true))
                .ReturnsAsync(new List<TreatmentResult> { new TreatmentResult(feature, "label", treatmentExpected, false, config: configExpected) });
        ...
        public async Task GetTreatmentWithConfig_WhenNameDoesntExist_ReturnsControl()
        {
            ...
            _evaluator
                .Setup(mock => mock.EvaluateFeaturesAsync(It.IsAny<Splitio.Enums.API>(), It.IsAny<Key>(), It.IsAny<List<string>>(), It.IsAny<Dictionary<string, object>>(), true))
                .ReturnsAsync(new List<TreatmentResult> { new TreatmentResult("not_exist", Labels.SplitNotFound, "control", false) });
        ...
        [TestMethod]
        public async Task FallbackTreatments_WhenFlagDoesNotExist()
        {
            ...
            _splitCache
                .Setup(mock => mock.FetchManyAsync(new List<string> { splitName, "feature" }))
                .ReturnsAsync(new List<ParsedSplit> { null });
            _splitCache
                .Setup(mock => mock.GetNamesByFlagSetsAsync(new List<string> { "set" }))
                .ReturnsAsync(new Dictionary<string, HashSet<string>> { { "set", new HashSet<string>() { splitName, "feature" } } });
            _blockUntilReadyService
                .Setup(mock => mock.IsSdkReady())
                .Returns(true);
            ...
            SplitResult splitResult = await _splitClient.GetTreatmentWithConfigAsync("key", splitName);
            Assert.AreEqual("off-local", splitResult.Treatment);
            Assert.AreEqual("\"prop\":\"local\"", splitResult.Config);

            Dictionary<string, string> treatments = await _splitClient.GetTreatmentsAsync("key", new List<string>() { splitName, "feature" });
            ...
            Dictionary<string, SplitResult> splitResults = await _splitClient.GetTreatmentsWithConfigAsync("key", new List<string>() { splitName, "feature" });
            ...
            treatments = await _splitClient.GetTreatmentsByFlagSetAsync("key", "set");
            treatments.TryGetValue(splitName, out treatment);
            Assert.AreEqual("off-local", treatment);
            treatments.TryGetValue("feature", out treatment);
            Assert.AreEqual("on-global", treatment);

            treatments = await _splitClient.GetTreatmentsByFlagSetsAsync("key", new List<string>() { "set" });
            treatments.TryGetValue(splitName, out treatment);
            Assert.AreEqual("off-local", treatment);
            treatments.TryGetValue("feature", out treatment);
            Assert.AreEqual("on-global", treatment);

            splitResults = await _splitClient.GetTreatmentsWithConfigByFlagSetAsync("key", "set");
            splitResults.TryGetValue(splitName, out splitResult);
            Assert.AreEqual("off-local", splitResult.Treatment);
            Assert.AreEqual("\"prop\":\"local\"", splitResult.Config);
            splitResults.TryGetValue("feature", out splitResult);
            ...
            splitResults = await _splitClient.GetTreatmentsWithConfigByFlagSetsAsync("key", new List<string>() { "set" });
            splitResults.TryGetValue(splitName, out splitResult);
            Assert.AreEqual("off-local", splitResult.Treatment);
            Assert.AreEqual("\"prop\":\"local\"", splitResult.Config);
            splitResults.TryGetValue("feature", out splitResult);
        ...
        [TestMethod]
        public async Task FallbackTreatments_WhenExceptionInClient()
        {
            Mock<ISplitter> _splitter = new Mock<ISplitter>();
            Mock<IFeatureFlagCache> _splitCache = new Mock<IFeatureFlagCache>();
            Mock<ITelemetryEvaluationProducer> _telemetryEvaluationProducer = new Mock<ITelemetryEvaluationProducer>();
